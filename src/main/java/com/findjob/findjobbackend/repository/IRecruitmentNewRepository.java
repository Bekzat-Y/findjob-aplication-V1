...
@Repository
public interface IRecruitmentNewRepository extends JpaRepository<RecruitmentNew, Long> {
    List<RecruitmentNew> findAllByCompany_Id(Long id);

    @Query(value = "SELECT COUNT(r) FROM RecruitmentNew r where (:title IS NULL or (r.title like CONCAT('%',:title,'%') or r.company.name like CONCAT('%',:title,'%') or r.city.name like CONCAT('%',:title,'%')))" +
