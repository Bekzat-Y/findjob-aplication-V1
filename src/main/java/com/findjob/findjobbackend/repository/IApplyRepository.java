...
public interface IApplyRepository extends JpaRepository<Apply, Long> {
    ...
    @Query(
            value = "select a.id as id, a.date as date, a.status as status, a.recuitment_new_id as recuitmentNewId, a.user_id as userId, r.title as title, r.vacancies_id as vacanciesId, v.name as vacanciesName, c.avatar as avatar, u.name as nameUser, u.phone as phoneUser " +
                    "from apply a " +
                    "join recuitmentnew r on a.recuitment_new_id = r.id " +
